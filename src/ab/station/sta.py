...
class Type003Row:
    """
    Data for a single line in section `TYPE 003: HANDLING OF STATION PROBLEMS`
    of a STA file.
    ...
    _fstr = (
        ...
        "      001  "
        # "{yyyy: >4d} {mm:0>2d} {dd:0>2d} 00 00 00"
        "{skip_period_begin:10s} 00 00 00"
        "  "
        "{skip_period_end:10s} 23 59 59"
        "  "
        "{remark: <60s}"
    )
...
def extract_type3_station_events(
    ...
    for prev_rec, rec in zip(receivers, receivers[1:]):
        ...
        if (
            ...
            skip = "day"  # firmware change requires skip day only
        else:
            continue
        events.append(
            {
                "skip_period": get_skip_period(
                    prev_rec.date_removed, rec.date_installed, skip
                ),
                "remark": remark,
            }
        )
    ...
    # if two changes made in the same interval, combine remarks and keep only one event
    for prev, curr in zip(events, events[1:]):
        if prev["skip_period"] == curr["skip_period"]:
            curr["remark"] = prev["remark"] + ", " + curr["remark"]
            events.remove(prev)

    return events


def map_to_type_3_row(
    station_id: str,
    domes: str,
    skip_period: list[str, str],
    remark: str,
    **_: t.Any,
) -> dict[str, str]:
    """
    ...
    return dict(
        station_name=station_name(station_id, domes),
        skip_period_begin=skip_period[0],
        skip_period_end=skip_period[1],
        remark=remark,
    )
...
def transform_sitelog_records_to_STA_lines(
    ...
    log.info(f"Build Type-003 lines for {sitelog.filename.name}")
    type_3_lines = []
    if skip_period:
        ...
        type_3_rows = [
            map_to_type_3_row(**{**constants, **parameters})
            for parameters in type_3_data
        ]
        type_3_lines = [Type003Row(**parameters) for parameters in type_3_rows]
...
def create_sta_file_from_sitelogs(
    ...
    data = dict(
        ...
        type_3_rows="\n".join(str(row) for row in type_3_rows),
